  ...
  return (
    ...
    >
      ...
      <div className="relative aspect-[4/3] w-full bg-transparent">
        {isProtocol ? (
          <span className="absolute left-3 top-3 z-10 rounded-full border border-white/20 bg-[#0B1D2C]/80 px-2.5 py-1 font-mono text-[10px] font-bold uppercase tracking-wider text-[#D8B657] backdrop-blur-md shadow-sm">
            Protocolo Clínico
          </span>
        ) : null}
        <Image
      ...
      <div className="flex flex-1 flex-col border-t-2 border-content/12 bg-card p-5">
        <p className="font-mono text-[11px] font-semibold uppercase tracking-[0.14em] text-(--line-accent)">{line.name} · {isProtocol ? "Protocolo Clínico" : item.category}</p>
        ...
        <div className="mt-auto pt-5">
          <span className="inline-flex w-full items-center justify-center gap-2 rounded-full bg-action px-5 py-3 text-sm font-semibold text-action-fg transition group-hover:bg-action-hover">
