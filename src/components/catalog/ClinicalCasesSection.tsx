...
export function ClinicalCasesSection({ cases }: { cases: CatalogClinicalCase[] }) {
  ...
  return <section className="mt-14 border-t border-content/10 pt-12" aria-labelledby="clinical-cases-title">
    ...
    <div className="mt-8 space-y-8">{cases.map((clinicalCase) => <article key={clinicalCase.id} className="rounded-[24px] border border-content/10 bg-card p-5 md:p-7">
      ...
      <div className="grid gap-4 sm:grid-cols-2">{[["Antes", clinicalCase.beforeImage], ["Depois", clinicalCase.afterImage]].map(([label, src]) => <figure key={label} className="overflow-hidden rounded-[18px] border border-content/10 bg-canvas"><div className="relative aspect-[4/3]"><Image src={src} alt={`${label} — ${clinicalCase.title}`} fill sizes="(max-width: 640px) 92vw, 42vw" className="object-contain" /></div><figcaption className="border-t border-content/10 px-4 py-2 font-mono text-xs font-semibold uppercase tracking-wider">{label}</figcaption></figure>)}</div>
