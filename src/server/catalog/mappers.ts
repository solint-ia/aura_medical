...
export function mapProduct(row: ProductRow): CatalogItem {
  ...
  return {
    ...
    imageFraming: toFraming(primaryImage, "card"),
    images: row.images.map((image) => ({ src: mediaUrl(image.asset) || "", alt: image.asset.alt, caption: image.caption || undefined, framing: toFraming(image.asset, "page") })),
...
export function mapProtocol(row: ProtocolRow): CatalogItem {
  ...
  return {
    ...
    imageFraming: toFraming(row.coverImage, "card"),
    images: gallery.map((image) => ({ src: mediaUrl(image.asset) || "", alt: image.asset.alt, caption: image.caption || undefined, framing: toFraming(image.asset, "page") })),
...
function mapClinicalCase(clinicalCase: CaseWithLinks, currentHref: string) {
  ...
  return {
    ...
    beforeFraming: toFraming(clinicalCase.beforeImage, "page"),
    afterFraming: toFraming(clinicalCase.afterImage, "page"),
