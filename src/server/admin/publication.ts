...
type ProtocolCandidate = { sessions: string; frequency: string; coverImageId: string | null; components: unknown[]; skus: { isActive: boolean }[]; line: { status: string } };
...
export function productPublicationError(product: ProductCandidate): string | null {
  ...
  if (!product.name.trim() || !product.summary.trim() || !product.presentation.trim()) return "Preencha nome, resumo e apresentação.";
